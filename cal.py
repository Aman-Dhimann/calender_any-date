...
month_separator = format(' ', '10')
blank_week = format(' ', '20')
blank_col = format(' ', '3')
...
while not terminate:
    ...
    if year == -1:
        ...
        for month_num in range(12):
            month_name = month_names[
                month_num]  # month name is retrieved from tuple month_names and assigned to month_name
    ...
    for month_num in [0, 3, 6, 9]:
        ...
        week = 0
        print()
        week_max = max(len(calendar_year[month_num]), len(calendar_year[month_num + 1]),
                       len(calendar_year[month_num + 2]))
        lines_to_print = True
